...
public class Util {
    ...
    protected static double toCoordFrameX(double posX) {
        posX = (posX - AppBoard.get().getOrigin().x) / AppBoard.get().getPixelsPerUnit();
        return posX;
    }
    protected static double toCoordFrameY(double posY) {
        posY = -(posY - AppBoard.get().getOrigin().y) / AppBoard.get().getPixelsPerUnit();
        return posY;
    }
    protected static Vec2 toCoordFrame(Vec2 pos) {
        Vec2 origin = AppBoard.get().getOrigin();
        return new Vec2((pos.x-origin.x) / AppBoard.get().getPixelsPerUnit(), -(pos.y-origin.y) / AppBoard.get().getPixelsPerUnit());
    }
    protected static double toCoordFrameLength(double len) {
        return len / AppBoard.get().getPixelsPerUnit();
    }
    protected static double toPixelsX(double posX) {
        Vec2 origin = AppBoard.get().getOrigin();
        posX = posX * AppBoard.get().getPixelsPerUnit() + origin.x;
        return posX;
    }
    protected static double toPixelsY(double posY) {
        Vec2 origin = AppBoard.get().getOrigin();
        posY = -posY * AppBoard.get().getPixelsPerUnit() + origin.y;
        return posY;
    }
    protected static int toPixelsXInt(double posX) {
        Vec2 origin = AppBoard.get().getOrigin();
        posX = posX * AppBoard.get().getPixelsPerUnit() + origin.x;
        return (int)Math.round(posX);
    }
    protected static int toPixelsYInt(double posY) {
        Vec2 origin = AppBoard.get().getOrigin();
        posY = -posY * AppBoard.get().getPixelsPerUnit() + origin.y;
        return (int)Math.round(posY);
    }    
    protected static Vec2 toPixels(Vec2 pos) {
        Vec2 origin = AppBoard.get().getOrigin();
        return new Vec2(pos.x * AppBoard.get().getPixelsPerUnit() + origin.x, -pos.y * AppBoard.get().getPixelsPerUnit() + origin.y);
    }
    protected static Vec2 toPixelDims(Vec2 dims) {
        return new Vec2(dims.x * AppBoard.get().getPixelsPerUnit(), dims.y * AppBoard.get().getPixelsPerUnit());
    }
    protected static double toPixelsLength(double len) {
        return len * AppBoard.get().getPixelsPerUnit();
    }
    protected static int toPixelsLengthInt(double len) {
        return (int)Math.round(len * AppBoard.get().getPixelsPerUnit());
    }
    ...
    public static Vec2 maxCoordFrameUnits() {
        return new Vec2(AppBoard.get().getCanvasSize().x / AppBoard.get().getPixelsPerUnit(), AppBoard.get().getCanvasSize().y / AppBoard.get().getPixelsPerUnit());
    }
    ...
    public static boolean isInsideField(Vec2 point, double buffer) {
        ...
        if ((point.x >= toCoordFrameLength(AppBoard.get().getCanvasSize().x) - buffer) || (point.y >= toCoordFrameLength(AppBoard.get().getCanvasSize().y) - buffer)) {
            return false;
        }
        return true;
    }
