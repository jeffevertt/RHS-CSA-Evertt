 ...
 */
public class NoiseMapPerlin implements NoiseMap {
    private double width = -1;
    private double length = -1;
    private double heightScale = -1;
    private static int perm[] = null;

    public NoiseMapPerlin(double width, double length, double heightScale) {
        this.width = width;
        this.length = length;
    ...
    public double calcValueAtPoint(Vec2 pt) {
        ...
        // Note: Typically Perlin noise output range is [-1,1], 
        //  rescaling to [0,1] before applying height scale here.
        double result = (Util.lerp(x1Inter, x2Inter, v) * 0.5 + 0.5) * heightScale;
        return result;
    }
