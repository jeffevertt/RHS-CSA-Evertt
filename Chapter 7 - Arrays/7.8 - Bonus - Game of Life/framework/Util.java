...
public class Util {
    ...
    public static double toCoordFrameX(double posX) {
        posX = (posX - GameBoard.get().getOrigin().x) / GameBoard.get().getPixelsPerCell();
        return posX;
    }
    public static double toCoordFrameY(double posY) {
        posY = -(posY - GameBoard.get().getOrigin().y) / GameBoard.get().getPixelsPerCell();
        return posY;
    }
    public static Vec2 toCoordFrame(Vec2 pos) {
        Vec2 origin = GameBoard.get().getOrigin();
        return new Vec2((pos.x-origin.x) / GameBoard.get().getPixelsPerCell(), -(pos.y-origin.y) / GameBoard.get().getPixelsPerCell());
    }
    public static double toCoordFrameLength(double len) {
        return len / GameBoard.get().getPixelsPerCell();
    }
    public static double toPixelsX(double posX) {
        Vec2 origin = GameBoard.get().getOrigin();
        posX = posX * GameBoard.get().getPixelsPerCell() + origin.x;
        return posX;
    }
    public static double toPixelsY(double posY) {
        Vec2 origin = GameBoard.get().getOrigin();
        posY = -posY * GameBoard.get().getPixelsPerCell() + origin.y;
        return posY;
    }
    ...
    public static Vec2 toPixels(Vec2 pos) {
        Vec2 origin = GameBoard.get().getOrigin();
        return new Vec2(pos.x * GameBoard.get().getPixelsPerCell() + origin.x, -pos.y * GameBoard.get().getPixelsPerCell() + origin.y);
    }
    public static Vec2 toPixelDims(Vec2 dims) {
        return new Vec2(dims.x * GameBoard.get().getPixelsPerCell(), dims.y * GameBoard.get().getPixelsPerCell());
    }
    public static double toPixelsLength(double len) {
        return len * GameBoard.get().getPixelsPerCell();
    }
    public static int toPixelsLengthInt(double len) {
        return (int)Math.round(len * GameBoard.get().getPixelsPerCell());
    }
    ...
    public static Vec2 maxCoordFrameUnits() {
        return new Vec2(GameBoard.get().getCanvasSize().x / GameBoard.get().getPixelsPerCell(), GameBoard.get().getCanvasSize().y / GameBoard.get().getPixelsPerCell());
    }
    ...
    public static boolean isInsideField(Vec2 point, double buffer) {
        ...
        if ((point.x >= toCoordFrameLength(GameBoard.get().getCanvasSize().x) - buffer) || (point.y >= toCoordFrameLength(GameBoard.get().getCanvasSize().y) - buffer)) {
            return false;
        }
        return true;
    }
